...
3) // Pokaż zwierzęta które "zawierają się" w boksie A4, B2
SELECT imie, waga, wiek FROM zwierzeta WHERE boks IN ('A4', 'B2');

4) // Informacja jakich karm dostarcza każdy z dostawców 
...
5) // Pokaż ile zwierząt konkretnego rodzaju jest w schronisku oraż średnią wagę dla danego rodzaju
SELECT rodzaj, COUNT(id_zwierzaka) AS liczbaZwierzat, AVG(waga) AS sredniaWaga FROM zwierzeta GROUP BY rodzaj;

6) // Pokaż karmy(producenta i smak), których schornisko ma na stanie mniej niż 100 kg
...
7) // Wypisz, które zwierzęta są odrobaczone, a które nie
...
8) // Wypisz dla karm liczbę jej konsumentów i ich imiona
...
9) // Wypisz zwierzęta razem z odpowiedzialnym za nich opiekunem
...
12) // Pokaż, które zwierzę lubi dziczyznę, a które nie
...
13) // Dla kazdego opiekuna wypisz ile ma podopiecznych i ich imiona
...
14) Wypisz informacje(o zwierzeciu i właścicielu) o adopcjach miedzy 2000 a 2010 rokiem. 
...
15) Pokaz niewykastrowane zwierzeta i lekarzy ktorzy beda musieli przeprowadzic ten zabieg
